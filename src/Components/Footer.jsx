...
export default function FooterMobile() {
  return (
    <footer data-nav="white" className="bg-black text-background pt-s160 pb-s16">

   <div className="flex flex-col md:flex-row justify-between space-y-s64 px-s32 ">
       {/* Pages */}
      <div className="space-y-s16">
        <h4 className="heading-h6  ">Pages</h4>
        <ul className="space-y-s8 body-default ">
          {footerLinks.pages.map(item => (
            <li key={item.label}>
              <Link className="hover:text-primary-main" href={item.path}>{item.label}</Link>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="space-y-s16">
        <h4 className="heading-h6  ">Social Media</h4>
        <ul className="space-y-s8 body-default ">
          {footerLinks.socialMedia.map((item, index) => (
            <li key={index}>
              <Link className="hover:text-primary-main" target="_blank" href={item.path}>{item.label}</Link>
            </li>
          ))}
        </ul>
      </div>
