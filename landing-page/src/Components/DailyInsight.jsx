...
function InsightCard({ text }) {
  return (
    ...
    >
      ...
      <span className="absolute right-4 bottom-4 text-green-400 text-3xl">
        ”
      </span>
    </div>
  );
...
function DailyInsights() {
  const InsightCard = useInViewOnce();
  ...
  return (
    <section className=" min-h-screen py-s160  flex justify-center items-center bg-black px-s16 ms:px-s32">
      <div className="space-y-s160">
        {/* Content */}
        <div className=" max-w-7xl mx-auto text-center">
          {/* Heading glass plate */}
          <div
            className=" rounded-r24 md:rounded-r32  px-s16
           backdrop-blur-xl border-[1px] border-white/30
        "
          >
            <h2
              ref={InsightCard.ref}
              className=" transition -translate-y-7 sm:-translate-y-15 lg:-translate-y-22
            text-6xl sm:text-[120px] lg:text-[160px] font-bold
            bg-gradient-to-b from-white to-/30
            bg-foreground bg-clip-text text-transparent animate-y-fadein 
          "
            >
              Daily Insights
            </h2>
